...
function showResume() {
    ...
    const backBtnLocal = document.getElementById('backToTopBtn');
    if (backBtnLocal) backBtnLocal.classList.add('visible');
...
function hideResume() {
    ...
    const backBtnLocal = document.getElementById('backToTopBtn');
    if (backBtnLocal) backBtnLocal.classList.remove('visible');
}

// Toggle helper to avoid conflicting listeners
...
function initializeApp() {
    ...
    // Initialize audio but do NOT auto-play; start only if user chooses sound
    ...
    (function initHeaderLogo(){
        ...
        // If the image is already cached/loaded, reveal immediately
    ...
    // Light particle field with fog-colored motes
    (function addFogParticles(){
        const hero = document.querySelector('.hero');
        if (!hero) return;
        const canvas = document.createElement('canvas');
        canvas.className = 'fog-particles';
