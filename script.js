...
function showResume() {
  ...
  fetch('assets/resume.json', { cache: 'no-store' })
    ...
    .then(data => {
      ...
      if (summaryEl && data.professionalSummary) {
        summaryEl.textContent = data.professionalSummary;
      }
...
function initializeApp() {
  ...
  if (entryOverlay && entryCircle && entryCircleLabel && enterNoSoundBtn) {
    ...
    // closeOverlay targeting #cabin audio
  ...
  // Contact button: scroll to Connect
  ...
  // Interactive features after a delay
...
// Additional interactive features (including snowflake cursor trail)
function addInteractiveFeatures() {
  // Scroll progress, parallax, tilt, etc.
  initInteractivity();

  // Minimal snowflake cursor trail (same throttle/size as leaf)
  let lastTrailAt = 0;
...
// Interactivity: scroll progress, parallax, tilt
function initInteractivity() {
  ...
  // Simple particle effect for background (hero)
  addParticleEffect();
}
